...
namespace beam
{
	...
	struct Rules
	{
		...
		struct {
			// emission parameters
			Amount Value0	= Coin * 275; // Initial emission. Each drop it will be halved. In case of odd num it's rounded to the lower value.
		...
		struct {
			...
			Difficulty Difficulty0	= Difficulty(0 << Difficulty::s_MantissaBits); // 2^22 = 4,194,304. For GPUs producing 7 sol/sec this is roughly equivalent to 10K GPUs.
		} DA;
