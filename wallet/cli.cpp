    ...
    int ShowWalletInfo(const IWalletDB::Ptr& walletDB, const po::variables_map& vm)
    {
        ...
        if (vm.count(cli::TX_HISTORY))
        {
            ...
            cout << "TRANSACTIONS\n\n  |"
                ...
                << right << setw(columnWidths[1]) << " amount, CASH" << " |"
        ...
        cout << "  |"
            ...
            << right << setw(columnWidths[1]) << " CASH" << " |"
            << setw(columnWidths[2]) << " Csats" << " |"
...
int main_impl(int argc, char* argv[])
{
    ...
    try
    {
        ...
        try
        {
            vm = getOptions(argc, argv, "litecash-wallet.cfg", options, true);
        }
        ...
        try
        {
...

           {
...

                {
                    ...
                    LOG_INFO() << "Litecash Wallet " << PROJECT_VERSION; // << " (" << BRANCH_NAME << ")";
